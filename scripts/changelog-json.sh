...
mapfile -t VERSIONS < <(git tag | grep -E "[0-9]+\.[0-9]+\.[0-9]+" | sort -Vr; git log --format=format:%H | tail -1)
...
for VERSION in "${VERSIONS[@]}"; do
    ...
    CHANGES="$(git diff --diff-filter=A --name-only "$VERSION" "$PREV_VERSION" -- changelog | sort -r)"
    export PREV_VERSION
